...
void setup() {
  ...
  count++;
  Serial.println(count);
  DDRB = 255; //se declara todo el puerto como salida
  Serial.begin(9600) ; // Inicializa el Puerto serie con 9600 bits/s.
  DDRB = DDRB | B11111100;   // selecciona la dirección de los bit 2 a 7, deja 0 y 1 sin tocar (xx | 00 == xx)
...
void loop() {
  ...
  for (i = 0; i < 64; i++) {
    ...
    PORTB = PORTB & B00000011;    // borra los bits 2 - 7, ldeja los pines 0 y 1 sin tocar (xx & 11 == xx)
    int   j = (i << 2);     // desplaza los pines 2 - 7 de la variable - para eludir los pines 0 y 1
   PORTB = PORTB | j;    // combina la información de los puertos con la nueva información para los pines LED
